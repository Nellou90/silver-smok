<script lang="ts">
  ...
  <div id="modalBackground" class="fixed inset-0 flex items-center justify-center bg-[#000000e1] bg-opacity-50" on:click={closeModal}>
      <form class="relative bg-[#e1e8ec] w-[25rem] h-[30rem] rounded-lg shadow-lg p-5">
          ...
          <button type="button" class="absolute top-3 right-3 p-2 text-gray-600 hover:bg-red-400 rounded-full hover:text-white transition" on:click={() => goto("/")}>
          ...
          <button on:click={login} type="submit" class="text-white bg-[#002b44] hover:bg-[#507e99] focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full px-5 py-2.5 text-center">Se connecter</button>
